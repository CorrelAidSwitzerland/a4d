...
hospital_split <- strsplit(input_excel_name, "AN ")[[1]]
hospital_split <- strsplit(hospital_split, " A4D")[[2]][1]
...
for(tab in unique(list_excel_tabs)){
  ...
  for(i in 1:nrow(df_alt1)){
    ...
    start_df_msd <- get_msd_start(df_alt1, i)
    end_df_msd <- get_msd_end(df_alt1, i)
    start_df_patient <- get_patient_start(df_alt1, i)
    end_df_patient <- get_patient_end(df_alt1, i)
