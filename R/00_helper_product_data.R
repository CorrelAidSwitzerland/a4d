...
extract_month <- function(sheetname){
  output <- case_when(grepl("Jan", sheetname) ~ "01",
...
compute_balance_status <- function(product_df){
  ...
  indices_start <- product_df %>%
    ...
    slice_head(n = 1) %>% as_tibble() %>% select(index) %>% unlist() %>%  as.numeric()
  
  indices_end <- product_df %>%
    dplyr::group_by(product) %>%
    slice_tail(n = 1) %>% as_tibble() %>% select(index) %>% unlist() %>%  as.numeric()
...
adjust_column_classes <- function(product_df){
  ...
  list_character <- c("product", "product_received_from", "product_released_to", "product_returned_by", "product_balance_status",
                      "product_country", "product_hospital", "product_sheet_name")
  list_numeric <- c("product_units_received", "product_units_released", "product_balance", "product_units_returned",
